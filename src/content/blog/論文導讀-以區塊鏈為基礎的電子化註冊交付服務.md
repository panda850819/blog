---
title: "論文導讀 — 以區塊鏈為基礎的電子化註冊交付服務"
description: "論文導讀 — 以區塊鏈為基礎的電子化註冊交付服務  BlockchainBased System for Multiparty Electronic Registered Delivery Services  這是一篇論文導讀的形式，會帶各位一步一步來探討論文的核心以其價值。 若翻譯上有任何的..."
publishedAt: 2019-09-06
tags: ["Blockchain", "Ethereum", "Tutorial", "Research"]
---

---

### 論文導讀 — 以區塊鏈為基礎的電子化註冊交付服務

Blockchain-Based System for Multiparty Electronic Registered Delivery Services

這是一篇論文導讀的形式，會帶各位一步一步來探討論文的核心以其價值。
若翻譯上有任何的問題，也請歡迎提 Isuue 或來跟我討論。

#### 原文在此

[**Blockchain-Based System for Multiparty Electronic Registered Delivery Services - IEEE Journals &…**
*European Regulation (EU) 910/2014 lays down the rules for electronic identification and trust services for electronic…*ieeexplore.ieee.org](https://ieeexplore.ieee.org/document/8764454)[](https://ieeexplore.ieee.org/document/8764454)#### 摘要（Abstract）

> 故事介紹

歐洲法規規定了電子化支付（電子化識別和信任服務）而一個合格的電子註冊交付是信託服務之一並包含在法規中，它要求**不可否認原產地**及**誠信的資料**。這些服務基本上會依賴於**信任的第三方**，仍會有很大的溝通及使用障礙。

**科普小學堂 **「**若想要讀得更詳細，請參閱最底下資料來源**」1. 交付：交付是動產、物權變動的公示方式。動產種類繁多，數量巨大，變動頻繁，不適宜採用登記方式。2. 信託：委託人將財產權移轉或為其他處分，使受託人依信託本旨，為受益人之利益或為特定之目的，管理或處分信託財產之關係。> eDelivery
...
區塊鏈提供了一個**不可修改的資料註冊系統**，可為傳統應用程式提供新的解決方案。

*舉例：經過****認證的****電子郵件帳號，可以經過區塊鏈技術來證明他已發送信件或其他類型的服務。*
...
2016年7月，歐盟信息安全網的法規 910/2014（也稱為 eIDAS 法規）開始適用。該法規為內部市場的電子交易制定了**電子識別**和**信託服務**規則，涵蓋了所有28個成員國。
...
> B. 交換協議中的安全性和可信任之第三方 
Fairness and TTPs in Exchange Protocols
...
Ideal Properties of a Registered eDelivery System
...
- [互操作性](https://zh.wikipedia.org/zh-tw/%E4%BA%92%E6%93%8D%E4%BD%9C%E6%80%A7)：服務向發送方指示預期接收方可以儲利的格式消息，並將消息轉換為另種格式。
...
- 降低風險：合格的電子註冊交付使得以下操作變得不可行（**操縱數據**、**偽造發送和接收的時間戳記**或**未授權訪問**）。
- 降低成本：降低失敗或不確定性的成本。
- 沒有傳輸延遲：傳輸必須幾乎是瞬間的。
- 沒有雙重支付：避免發送額外的簽名版本。
...
State of the Art of Registered eDelivery Services
...
- 圖1描述了非機密協議的參與者之間的鏈間交互
- 圖2中描述了機密協議的離線交互。

![](https://cdn-images-1.medium.com/max/800/0*hoH_i31v86P7XouY.gif)圖1 .非機密協議中的參與者之間的互動![](https://cdn-images-1.medium.com/max/800/0*gXXdJJlNDa2sBhzD.gif)圖2 . 機密協議中的參與者之間的互動---

### 基於區塊鏈的多方註冊 eDelivery 協議

Non-Confidential Blockchain-Based Multiparty Registered eDelivery Protocol

此方法非常適合需要儲存交付數據的應用程序，這些應用程序必須註冊並可公開訪問。

非機密註冊 eDeliveries 的多方協議提供了一個發送方（A）和多個接收方（B = {B1，B2 …，Bn}）的公平交付解決方案。

圖3是發送方和接收方在非機密 eDelivery 中為所提出的鏈上通信方案交換的消息序列圖。

在圖3中，藍色箭頭表示上述步驟，並指定對區塊鏈地址的簽名請求。
此外，框內的紅色文本描述了必須由區塊鏈上部署的 eDelivery 執行的過程。

![](https://cdn-images-1.medium.com/max/800/0*4HcAyLe3NGAbAKM0.gif)圖3 基於非機密區塊鏈的協議描述。交付的發送方 A 和接收方集合 B 將遵循交換協議的步驟。
...
term2 為允許接收方取消，未完成交付的期限。- 取消的功能是為了保證**有效性和公平性。**
- B 中的每個接收方 Bi 必須在 term1 期間單獨接受傳遞的接收，發布表達其意願的消息。

> 簽署的交易將作為接收證明的**不可否認性**。

- 如果接收方在 term1 期間不接受，則假定**拒絕**。
...
![](https://cdn-images-1.medium.com/max/800/0*mP-2hoj801HXi4Cp.gif)表1 非機密協議之要素---

### 基於機密區塊鏈的註冊 eDelivery 協議

Confidential Blockchain-Based Registered eDelivery Protocol
...
另一個重要特徵是該提案不需要截止日期，任何交換都可以隨時完成。
無國籍 TTP 可用於解決雙方之間可能產生的爭議。

我們描述了一種基於非區塊鏈的公平交換，我們將部分重用（即鏈下 3-Steps 交換），並在套用在本節中基於區塊鏈的新提案。

在新協議中，發送方 A 和收件方 B 直接交換消息和不可否認證據，使用圖4 中描述的 3-Steps 脫鏈通信。僅作為最後手段，以防他們無法獲得預期通過發送取消請求（圖5）或完成請求（圖6），可以調用來自另一方，智能合約或TTP的項目。

描述的協議相比，基於區塊鏈的解決方案中 TTP 的作用已大大降低。

此外，發送方永遠不會聯繫 TTP。
在這個新提案中，TTP 將僅採用已部署的智能合約來回應 B 的請求。
TTP 完全是無狀態的，因此它從不存儲有關任何交換狀態的信息。

> 圖中的藍色箭頭表示對區塊鏈地址的簽名請求，而黑色箭頭表示離線通信消息。

![](https://cdn-images-1.medium.com/max/800/0*w7Ns-wh6Gl4N8Gul.gif)**表2 機密多方協議之要素**#### 多重之正向交換子協議
...
![](https://cdn-images-1.medium.com/max/800/0*jcQTTeiMx_Dts0JN.gif)圖4交換如下：發送方向接收方集合發送消息，包括加密消息，接收者的地址和原始證據的不可否認性的第一部分，kT、hA。
...
> 這樣，協議允許正向交換，也就是說，交換可以完全執行而無需 TTP和區塊鏈的干預。
另一個重要特徵是該提案不需要截止日期，並且可以隨時完成，如果雙方之間發生爭議，可以執行以下子協議。
...
![](https://cdn-images-1.medium.com/max/800/0*UiCafbgW0l-YsekW.gif)圖5#### C. 多方完成子協議
...
![](https://cdn-images-1.medium.com/max/800/0*i_XvJZLSt0fyMtQy.gif)圖6---
...
Smart Contract for the Non-Confidential Multiparty Registered eDelivery Protocol

![](https://cdn-images-1.medium.com/max/800/0*cjq3ymOvTd4AcGw8.gif)Variables 屬性定義![](https://cdn-images-1.medium.com/max/800/0*lUww0uTcgUztutd_.gif)接受 function![](https://cdn-images-1.medium.com/max/800/0*x8HiiGbBcVQa8zjR.gif)完成 function![](https://cdn-images-1.medium.com/max/800/0*0B0ddiGfTbSAIbQH.gif)取消 function#### 機密多方註冊eDelivery協議的智能合約

Smart Contract for the Confidential Multiparty Registered eDelivery Protocol

![](https://cdn-images-1.medium.com/max/800/0*GCABQMtjGV8txvPj.gif)屬性以及建構子![](https://cdn-images-1.medium.com/max/800/0*XF3D8zMBOqzv25un.gif)取消 function![](https://cdn-images-1.medium.com/max/800/0*88urER8NTxG2-16G.gif)完成 function![](https://cdn-images-1.medium.com/max/800/0*whEFGxpvtcoQmQVf.gif)新增接收方 function![](https://cdn-images-1.medium.com/max/800/0*XYDo_CD6jAsGBjaD.gif)非機密協議中的狀態生命週期---
...
- [作者團隊 Github](https://github.com/secomuib)
